  ...
  let buildData = new BuildData(
    _.extend(
      {},
      {
        ...
        supplement: {
          source:
            'https://docs.google.com/spreadsheets/d/e/2PACX-1vSLghsM9-0PotsE7bLpXWS4bEMMYm7BpBxfDzUaN3GAdeMMVmn-smtu7U1IwgVznw6HQs_kKU_KDIKc/pub?output=csv',
          local: 'results-supplement.json',
          localOutput: 'assets/data',
          postprocess: data => {
            let parsed = _.mapValues(_.keyBy(_.filter(data, 'id'), 'id'), d =>
              _.omit(d, ['', null, undefined])
            );
            return parsed;
          }
        }
